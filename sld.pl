...
start:-
	...
	% ------------------------------------------------------------------------------
	loadbi :- load_program_intern('mod/builtin.pl').
...
sld_root(CurrentNodeId, Request, Tree, NewRootNode):-
	...
	FullPath = [p(goal(Goal), Vars, _)|_],
	 \+ Goal = '-',
	NewRootNode = node(id(0, Request),
		goal(Goal),
		cl(0,
			variant(true)),
		[],
		[loc(pid(0, Request), 0), FullPath],
		Vars).
